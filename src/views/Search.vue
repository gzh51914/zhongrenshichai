...
export default {
  ...
  mounted () {
    // this.$store.commit("hide")//触发mutations里面的hide()方法
    this.hide()
    if (this.cinemaList.length > 0) {
      // console.log("search-直接使用缓存vuex数据")
    } else {
      // this.$store.dispatch("getCinemaActions")
      this.getCinemaActions()
    }
  },
  beforeDestroy () {
    // this.$store.commit("show")//触发mutations里面的show()方法
    this.show()
  },
  methods: {
